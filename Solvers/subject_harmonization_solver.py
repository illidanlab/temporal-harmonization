...
from Models.model import MLP_harmonization, CustomDataset
import torch.nn.functional as F
import pickle

class subject_harmonization_solver(Solver_Base):
    
    def __init__(self, cfg_proj, cfg_m, name = "subject_harmonization"):
    ...
    def run(self, x_train, y_train, g_train, x_test, y_test, g_test, seed):
        ...
        model = MLP_harmonization(input_dim = len(x_train[0]), sbj_dim = len(list(set(g_train))), task_in_dim = len(x_train[0]), task_out_dim = self.cfg_m.data.dim_out, model_name = self.cfg_proj.model_name) 
        ...
        model, loss_train_trace = self.sbj_train(model, dataloader_train, None, optimizer_sbj, lr_scheduler_sbj, epochs = self.cfg_m.training.epochs_whiting)
        
        # Save harmonized feature
        if self.cfg_proj.save_harmonized_features:
    ...
    def sbj_train(self, model, dataloader_train, criterion, optimizer, lr_scheduler, epochs = 100):
        loss_train_trace = []
        ...
        for epoch in range(epochs):
            ...
            for train_X, train_Y, train_G, idx in dataloader_train:   
                train_X, train_Y, train_G = train_X.float().to(self.device), train_Y.long().to(self.device), train_G.to(self.device)
                
                [features, logits_sbj, logits_cog] = model(train_X, id = "0,1,2") #
                loss_sbj_mse = loss_mse(train_X, features)
                loss_sbj = 5 - loss_cross_ent_sbj(logits_sbj, train_G)
                loss_cog = loss_cross_ent_cog(logits_cog, train_Y)
                
                
                loss = loss_cog*0.5 + loss_sbj*0.25 + loss_sbj_mse*0.25 
