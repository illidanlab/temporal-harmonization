...
class Solver_Base:
    ...
    def save_results(self):
        ...
        info_ = "step-%d, sbj:auc=%.3f\u00B1%.3f,f1=%.3f\u00B1%.3f,sens=%.3f\u00B1%.3f,spec=%.3f\u00B1%.3f"%(-1,
            np.mean(AUC[:, 1]), np.std(AUC[:, 1]), np.mean(F1[:, 1]), np.std(F1[:, 1]), 
            np.mean(Sens[:, 1]), np.std(Sens[:, 1]), np.mean(Spec[:, 1]), np.std(Spec[:, 1]))
        print(info_)
        self.logger.info(info_)
        self.logger.info("AUC  = %.3f\u00B1%.3f"%(np.mean(AUC[:, 1]), np.std(AUC[:, 1])))
        self.logger.info("F1   = %.3f\u00B1%.3f"%(np.mean(F1[:, 1]), np.std(F1[:, 1])))
        self.logger.info("Sens = %.3f\u00B1%.3f"%(np.mean(Sens[:, 1]), np.std(Sens[:, 1])))
        self.logger.info("Spec = %.3f\u00B1%.3f"%(np.mean(Spec[:, 1]), np.std(Spec[:, 1])))
        ...
        return np.mean(np.array(self.performance_main["AUC"])[:, 0]), np.std(np.array(self.performance_main["AUC"])[:, 0])
