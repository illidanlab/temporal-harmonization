...
for task in ["Subject", "Age", "Edu", "Sex"]:
    ...
    for model_name in ["GRU", "LSTM"]:
        ...
        for seed in range(100):
            ...
            harmonized_model = torch.load(f"weights/temporal_sequence_harmonization_adversarial_solver_{model_name}.pt",weights_only=False).to(device)
            ...
            for epoch in range(num_epochs):
                ...
                for x_batch, y_batch in dataloader_train:
                    ...
                    harmonized_model.eval()
                    with torch.no_grad():
                        features = harmonized_model(x_batch, id = "0") #
                    model.train()
                    # Forward pass
        ...
        print(model_name, np.mean(accs), np.std(accs))